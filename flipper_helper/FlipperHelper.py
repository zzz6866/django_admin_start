...
if any([platform.startswith(os_name) for os_name in ["linux", "darwin", "freebsd"]]):
    ...
    from zugbruecke.wintypes import HWND, DWORD
elif platform.startswith("win"):
    import ctypes
    from ctypes.wintypes import HWND, DWORD
else:
    logger.info("UNSUPPORTED OS : " + platform)
...
class FlipperHelper:
    ...
    def login(self, media_type, user_type, sz_id, sz_pw, sz_cert_pw):  # 로그인
        logger.info("LOG IN")
        login = self.wmca.wmcaConnect
        login.argtypes = (HWND, DWORD, ctypes.c_char, ctypes.c_char, ctypes.c_char, ctypes.c_char, ctypes.c_char)
        login.restype = ctypes.c_bool
        login.wmcaConnect()
        return True
    ...
    def is_login(self):  # 로그인 체크
        return self.wmca.wmcaIsConnected()

    def logout(self):  # 로그아웃
        logger.info("LOG OUT")
        self.wmca.wmcaDisconnect()

    def get_tr(self, tr_id):  # 주가 조회
        logger.info("get TR")

    def get_stock(self, stock_id):  # 종목 조회
        logger.info("get TR")

    def set_flipper(self, 구매주, 손절, 익절, ):
        logger.info("set_flipper")
